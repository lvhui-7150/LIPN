...
# @torch.no_grad()
# def evaluate_phase_equivariance(model, device, num_samples=10):
#     model.eval()
#     # 设置你想要测试的角度（例如：从0到360度，每隔45度测一次）
#     # 也可以使用 np.linspace(0, 2*np.pi, 20) 测试更多精细角度
...
#             z = torch.randn(1, 64, 64, dtype=torch.complex64, device=device)
#
#             # 1. 计算 f(z * e^iφ)
#             out_rotated_input = model.denoiser(z * phase_factor)
#
#             # 2. 计算 f(z) * e^iφ
#             out_raw = model.denoiser(z)
#             out_rotated_output = out_raw * phase_factor
#
#             # 3. 相对误差
#             err = torch.norm(out_rotated_input - out_rotated_output) / (torch.norm(out_rotated_output) + 1e-8)
#             errors.append(err.item())
#
#         results[phi_val] = np.mean(errors)
#         print(f"Angle {np.degrees(phi_val):>3.0f}° | Error: {results[phi_val]:.2e}")
#
#     return results
...
if __name__ == "__main__":
    ...
    nontrainable_models = {
        ...
        "CPNN": CPNN(K=5, eta=0.5, lam=0.1, noise_std=0)
    }
